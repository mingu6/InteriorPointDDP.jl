function setup_concar(; horizon::Int=501, dt::Float64=0.1, x1::Vector{Float64}=[1.0; 1.0; pi*3/2; 0.0])
    N = horizon
    num_state = 4
    num_action = 2
    h = dt
    
    function concar_aux(x, u)
        d = 2.0  
        return [
    ...
    concar = Dynamics(concar_dynamics, num_state, num_action)
    dynamics = [concar for k = 1:N-1]
    
    stage = Cost((x, u) -> stage_cost(x,u), num_state, num_action)
    objective = [
        [stage for k = 1:N-1]..., Cost((x, u) -> final_cost(x), num_state, 0)
    ]
    ...
    constraints = [
        [stage_constr for k = 1:N-1]..., Constraint() # no terminal constraint 
    ]
