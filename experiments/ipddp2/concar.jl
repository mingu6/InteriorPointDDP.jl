...
num_primal = num_action + num_obstacles + num_state  # 2 slacks for box constraints
...
f = (x, u) -> u[num_action + num_obstacles .+ (1:num_state)]
...
obs_dist(obs_xy) = (x, u) -> begin
    xp = u[num_action + num_obstacles .+ (1:2)]
...
[
    ...
    [h * ((obs[3] + r_car)^2 - obs_dist(obs[1:2])(x, u) + u[num_action + i])
        for (i, obs) in enumerate(xyr_obs)];
    # # bound constraints, car must stay within [0, 1] x [0, 1] box
    # u[num_action + num_obstacles + (1:2)];
    h * (RK4(x, u, g) - u[num_action + num_obstacles .+ (1:num_state)])
]
end
...
open(fname, "w") do io
    ...
    for seed = 1:50
        ...
        ū = [[T(1.0e-1) .* (rand(T, 2) .- 0.5); T(0.01) * ones(T, num_obstacles); xs_init[k][1:2]; (rand(T, 2) .- 0.5) .* 0.1] for k = 1:N-1]

        # ū[1][end-num_state+1:end] .= RK4(x1, ū[1][1:num_action], g)
        # for k = 2:N-1
        #     ū[k][end-num_state+1:end] .= RK4(ū[k-1][end-num_state+1:end], ū[k-1][1:num_action], g)
        # end
        solve!(solver, x1, ū)
